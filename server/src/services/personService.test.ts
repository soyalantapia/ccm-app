...
describe('el buscador encuentra por lo que promete', () => {
  ...
  beforeEach(async () => {
    ...
    // Antes esto era `findFirst()` + `if (!cv) return`: el bloque dependía de que ALGÚN otro test
    // (o el seed) hubiera dejado una convocatoria en la base. En una base recién creada no hay
    // ninguna, así que el `return` silencioso se saltaba toda la preparación y estos tests
    // fallaban como si el buscador estuviera roto. Peor: pasaban o fallaban según el orden de los
    // archivos, que es la peor forma de fallar. Ahora se crea lo que necesita.
    const cv =
