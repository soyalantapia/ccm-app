...
interface RegistrationRowProps {
  ...
  showQrLink?: boolean
  /** Muestra el botón "Agregar al calendario" (Mi QR). */
 ...
 */
export function RegistrationRow({ registration, showQrLink, showCalendar }: RegistrationRowProps) {
  ...
  return (
    <article className="border-t border-line py-4">
      ...
      {showCalendar && <AddToCalendar event={event} className="mt-3" />}
    </article>
  )
}
